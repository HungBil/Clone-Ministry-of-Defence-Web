...
Build a Vercel-ready clone of `https://bqp.vn/` for school work:

- Crawl the homepage, article pages, article content, metadata, and visual layout references.
...
- Use a real OpenAI API key only through local or deployed environment variables.
...
- Do not commit `.local-ai/`, `.claude/`, `.codex/`, `.env`, or `.env.local`.
- Do not push crawled caches or generated local-only agent files unless explicitly requested.
